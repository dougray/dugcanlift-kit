...
Usage (from the package root, with lift-ios checked out beside this repo):
    python3 Tools/build_reference.py \
        --sources ../lift-ios/data --out Sources/LiftReference/Resources

WHERE THIS LIVES
----------------
Copied from `lift-ios/Tools/build_reference.py` on 2026-09-16, when saturated
fat was added. The files it writes have lived in this package since LiftKit
was extracted, so this is now the copy to change. The raw sources are not
checked in anywhere: the USDA CSVs (`fdc/food.csv`, `fdc/food_nutrient.csv`,
a 36 MB SR Legacy + Foundation extract) and `exercises/free-exercise-db.json`
sit untracked under `lift-ios/data/`. No `off/` export was present for the
shipped build, so `food.db` holds USDA rows only.

Rebuilding from those sources reproduces the shipped `foods` table exactly --
same 7,928 rows, same ids, same rowids, same values -- which was checked
before the saturated-fat column was added, so the only change that column
brought is the column. (The file's bytes differ between builds regardless;
SQLite page layout is not a content guarantee.)
    ...
    with path.open(newline="", encoding="utf-8", errors="replace") as handle:
        ...
        for row in reader:
            ...
            batch.append((
                ...
                to_float(row.get("saturated-fat_100g")),
                "openfoodfacts",
            ))
